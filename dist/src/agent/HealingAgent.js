...
const ClaudeClient_1 = require("../llm/ClaudeClient");
...
class HealingAgent {
    constructor(config, registry) {
        ...
        this.claudeClient = new ClaudeClient_1.ClaudeClient(config);
    }
    async heal(context) {
        ...
        let candidates = [];
        try {
            const focusedSnapshot = DOMAnalyzer_1.DOMAnalyzer.extractRelevantSubtree(context.domSnapshot, context.originalSelector, this.config.domSnapshotMaxChars);
            const prompt = PromptBuilder_1.PromptBuilder.buildHealingPrompt({
                ...context,
